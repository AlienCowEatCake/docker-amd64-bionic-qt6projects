...
RUN export CMAKE_VERSION="3.29.6" && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" ./configure --prefix=/opt/cmake --no-qt-gui --parallel=$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make install && \
...
RUN export NINJA_VERSION="1.12.1" && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" cmake --build build --target ninja -- -j$(getconf _NPROCESSORS_ONLN) && \
    strip --strip-all build/ninja && \
    cp -a build/ninja /usr/local/bin/ && \
    cd .. && \
    rm -rf v${NINJA_VERSION}.tar.gz ninja-${NINJA_VERSION}

RUN export CLANG_VERSION="18.1.8" && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" cmake -S llvm -B build \
        -G "Ninja" \
        -DCMAKE_BUILD_TYPE=Release \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" cmake --install build --prefix "/opt/clang" && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" cmake -S runtimes -B build_runtimes \
        -G "Ninja" \
        -DCMAKE_BUILD_TYPE=Release \
        ...
        -DLIBCXX_INCLUDE_BENCHMARKS=OFF \
        -DCMAKE_INSTALL_PREFIX="/opt/clang" && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" cmake --build build_runtimes --target cxx cxxabi unwind && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" cmake --build build_runtimes --target install-cxx install-cxxabi install-unwind && \
    cd .. && \
...
RUN export XCB_PROTO_VERSION="1.17.0" && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" ./configure --prefix=/opt/xcb PYTHON=python3 && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make install && \
    cd .. && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" ./configure --prefix=/opt/xcb --disable-shared --enable-static --with-pic --without-doxygen --enable-ge --enable-xevie --enable-xprint --enable-selinux PKG_CONFIG_PATH=/opt/xcb/share/pkgconfig CFLAGS="-O3 -DNDEBUG" PYTHON=python3 && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make install && \
    cd .. && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" ./configure --prefix=/opt/xcb --disable-shared --enable-static --with-pic --without-doxygen PKG_CONFIG_PATH=/opt/xcb/lib/pkgconfig CFLAGS="-O3 -DNDEBUG" && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make install && \
    cd .. && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" ./configure --prefix=/opt/xcb --disable-shared --enable-static --with-pic --without-doxygen PKG_CONFIG_PATH=/opt/xcb/share/pkgconfig CFLAGS="-O3 -DNDEBUG" PYTHON=python3 && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make install && \
    cd .. && \
...
RUN export OPENSSL_VERSION="3.3.1" && \
    export OPENSSL_DEBIAN_VERSION="3.3.0-1" && \
    wget --no-check-certificate https://www.openssl.org/source/openssl-${OPENSSL_VERSION}.tar.gz && \
    wget --no-check-certificate https://snapshot.debian.org/archive/debian/20240531T083821Z/pool/main/o/openssl/openssl_${OPENSSL_DEBIAN_VERSION}.debian.tar.xz && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" ./Configure linux-$(gcc -dumpmachine | sed 's|-.*||' | sed 's|^i686$|x86| ; s|^arm$|armv4| ; s|^powerpc64le$|ppc64le|') --prefix=/opt/openssl --openssldir=/etc/ssl zlib no-shared && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make depend && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make install && \
    cd .. && \
...
RUN export QT_VERSION="6.7.2" && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" ../configure -prefix /opt/qt6 -opensource -confirm-license \
        ...
        && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" cmake --build . --parallel && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" cmake --install . && \
    cd ../.. && \
...
RUN export QT6GTK2_COMMIT="d29ba6c1fb4ac933ed7b91f0480cbd0c5a975ab8" && \
    ...
    mkdir build && \
    cd build && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" /opt/qt6/bin/qmake -r CONFIG+=release ../qt6gtk2.pro && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make install && \
    cd ../.. && \
...
RUN export QT6CT_COMMIT="55dba8704c0a748b0ce9f2d3cc2cf200ca3db464" && \
    ...
    mkdir build && \
    cd build && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" /opt/qt6/bin/qmake -r CONFIG+=release ../qt6ct.pro && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make install && \
    cd ../.. && \
...
RUN export APPIMAGEKIT_VERSION="13" && \
    ...
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) 7z && \
    cd .. && \
...
RUN export PATCHELF_VERSION="0.18.0" && \
    ...
    LDFLAGS="-fuse-ld=lld" \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" ./configure --prefix=/usr/local && \
    setarch "$(gcc -dumpmachine | sed 's|-.*||')" make -j$(getconf _NPROCESSORS_ONLN) && \
